...
static inline void apsUpdateFrame(davisCommonHandle handle, uint16_t data) {
	...
	if (IS_DAVIS640H(handle->info.chipID)) {
		if (state->aps.cDavisSupport.offsetDirection == 0) {
			// Increasing
			state->aps.cDavisSupport.offset++;

			if (state->aps.cDavisSupport.offset == ((state->aps.roi.sizeX / 2) + 1)) {
				// Switch to decreasing after last even pixel.
				state->aps.cDavisSupport.offsetDirection = 1;
				state->aps.cDavisSupport.offset          = I16T((state->aps.roi.sizeX / 2) - 2);
			}
		}
		else {
			// Decreasing
						...
						case 10: {
							...
							if (validFrame) {
								...
								if (ensureSpaceForEvents((caerEventPacketHeader *) &state->currentPackets.frame,
									...
									if (IS_DAVIS640H(handle->info.chipID)) {
										// Copy fourth quadrant (DAVIS) from first (G pixel), as DAVIS readout is
										// broken due to design error and has unuseable quality.
										uint16_t *pixels
											= caerFrameEventGetPixelArrayUnsafeConst(state->aps.frame.currentEvent);
										for (size_t y = 0; y < state->aps.roi.sizeY; y++) {
											for (size_t x = 0; x < state->aps.roi.sizeX; x++) {
												if (((x % 2) == 0) && ((y % 2) == 0)) {
													pixels[((y + 1) * state->aps.roi.sizeX) + (x + 1)]
														= pixels[(y * state->aps.roi.sizeX) + x];
												}
											}
										}
									}

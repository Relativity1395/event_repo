			...
			switch (paramAddr) {
				...
				case DVX_DVS_CHIP_CROPPER_X_START_ADDRESS: {
					...
					if (state->dvs.flipX) {
						...
						state->dvs.cropperXEnd = U16T(param);
						...
						state->dvs.cropperXStart = U16T(param);
	...
	switch (modAddr) {
		...
		case DVX_DVS_CHIP:
			switch (paramAddr) {
				...
				case DVX_DVS_CHIP_DUAL_BINNING_ENABLE: {
					...
					if (!spiConfigReceive(&state->usbState, DEVICE_DVS, REGISTER_DIGITAL_DUAL_BINNING, &currVal)) {
						return (false);
					}
		...
		case DVX_DVS_CHIP_CROPPER:
			switch (paramAddr) {
				...
				case DVX_DVS_CHIP_CROPPER_Y_START_ADDRESS: {
					if (state->dvs.flipY) {
						*param = U32T(handle->info.dvsSizeY) - 1 - state->dvs.cropperYEnd;
					}
					else {
				...
				case DVX_DVS_CHIP_CROPPER_Y_END_ADDRESS: {
					if (state->dvs.flipY) {
						*param = U32T(handle->info.dvsSizeY) - 1 - state->dvs.cropperYStart;
					}
					else {
				...
				case DVX_DVS_CHIP_CROPPER_X_START_ADDRESS: {
					if (state->dvs.flipX) {
						*param = U32T(handle->info.dvsSizeX) - 1 - state->dvs.cropperXEnd;
					}
					else {
						*param = state->dvs.cropperXStart;
					}
					break;
				}
				...
				case DVX_DVS_CHIP_CROPPER_X_END_ADDRESS: {
					if (state->dvs.flipX) {
						*param = U32T(handle->info.dvsSizeX) - 1 - state->dvs.cropperXStart;
					}
					else {
						*param = state->dvs.cropperXEnd;
					}
					break;
				}
	...
	for (size_t bufferPos = 0; bufferPos < bufferSize; bufferPos += 2) {
		...
		if ((event & 0x8000) != 0) {
			...
			switch (code) {
				...
				case 1: {
					// X column address. 10 bits (9 - 0) contain address, bit 11 Start of Frame marker.
